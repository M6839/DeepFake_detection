...
streamlit run app.py                   # interface for uploading the video
```
...
| **3** | `03-train_cnn.py` | Train EfficientNet B0 backbone → global max pooling → 2× FC (ReLU) → sigmoid. Input 128×128 RGB; output probability pristine (1) vs deepfake 
(0). |
| **4**| `streamlit app.py `| Web interface for uploading videos and showing the result.
...
**How do I detect if an image is a deepfake?**  
 run the trained model on a face crop (128×128). The model outputs a score: higher = more likely pristine, lower = more likely synthetic.
...
- **[M Kiran Kumar](https://github.com/M6839)** — *Initial work & Maintenance*
